...
final class Factories {
    ...
    private static ModuleDashboard assignScale(IModuleDashboard module) {
        ...
        moduleDashboard.setPoints(0);//TODO:
        moduleDashboard.setScore(module.getScorePerformanceActivity());
        if (DEFAULT_MODULES.contains(module.getId())) {
            moduleDashboard.setScale(defaultScale(module.getScorePerformanceActivity().intValue()));
        } else {
            moduleDashboard.setScale(scaleSocialEmotional(module.getScorePerformanceActivity().intValue()));
        }
